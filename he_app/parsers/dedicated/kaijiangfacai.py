...
def _deduplicate_blocks(blocks: list[TableBlock]) -> list[TableBlock]:
...
def _table_blocks(documents: list[str]) -> list[TableBlock]:
    ...
    for document in documents:
        ...
        for title in soup.select(".list-title"):
            ...
            next_title = title.find_next(class_="list-title")
            if table is None or (
                next_title is not None and tag_order[id(next_title)] < tag_order[id(table)]
            ):
                continue
                ...
                block.append(
                    (
                        ...
                        Candidate(
                            values=value,
                            line=normalize_text(
                                f"{row_period}期:开奖发财 综合杀料 杀合[{value}] "
                                + " | ".join(cells)
                            ),
                            score=120,
                            order=len(block),
    ...
    if len(unique) > 1:
        candidates = [candidate for block in unique for _period, candidate in block]
...
def find_kaijiangfacai_kill_sum_candidate_with_direction(
    documents: list[str], period: int, pick: str = "bottom"
) -> tuple[Candidate | None, bool]:
    blocks = _table_blocks(documents)
    target_found = any(
        row_period == period for block in blocks for row_period, _candidate in block
    )
    if not blocks:
    ...
    return match, target_found and match is None


def extract_kaijiangfacai_kill_sum_period_values(
    documents: list[str],
) -> dict[int, str]:
    values: dict[int, str] = {}
    for block in _table_blocks(documents):
