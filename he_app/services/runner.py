...
def _run_threaded_site_jobs(
    ...
    *,
    ...
    outcomes = {}
    timings = []
    browser_pool_size = default_browser_pool_size([site for _index, site in all_sites], workers)
    ...
    browser_pool = BrowserPool(browser_pool_size, headless=not show_browser) if browser_pool_size else None
    ...
    try:
        with ExitStack() as executors:
            ...
            browser_executor = (
                executors.enter_context(ThreadPoolExecutor(max_workers=browser_pool_size))
                if split_pools
                else executor
            )
            future_map = {}
            ...
            for index, site in all_sites:
                print(f"{format_progress_prefix(index, total)} [START] {site.name} ({site.pick})")
            ...
            for future in as_completed(future_map):
                ...
                index, site, result, detail, error, values, previous, elapsed = consume_site_future(
    ...
    *,
    ...
    def on_result(job: IsolatedJobResult) -> None:
        if job.error is not None:
            ...
            outcome = job.value
            if not isinstance(outcome, tuple) or len(outcome) != 6 or outcome[0] != job.site:
                outcome = (
                    job.site,
                    None,
                    "",
...
def run(args: argparse.Namespace) -> int:
    ...
    for index, site in all_sites:
        ...
        returned_site, result, detail, error, _values, _previous = outcome
        if returned_site != site:
            outcome = (site, None, "", "RuntimeError: 结果站点身份不一致", [], None)
            outcomes[index] = outcome
            returned_site, result, detail, error, _values, _previous = outcome
        completed_count += 1
        success_count += int(bool(result))
        fail_count += int(not result)
        elapsed = next((value for name, value, _ok in site_timings if name == site.name), 0.0)
        print_outcome(site, result, detail, error, format_progress_prefix(index, total_progress))
        print(
            format_completion_progress(
                completed_count,
                total_progress,
