...
from he_app.services.crawler import build_mirror_url_map, print_outcome, scrape_parallel_site
...
from he_app.storage.reports import build_current_only_results, build_success_output_lines, format_failure_output


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="抓取绝杀一合的合数，并分别输出成功和失败 txt。")
    parser.add_argument("--period", type=int, default=125, help="要抓取的期数，例如 120")
    ...
    parser.add_argument("--cache", default=CACHE_FILE, help=argparse.SUPPRESS)
    parser.add_argument("--mirror-limit", type=int, default=0, help="原地址请求失败时尝试的备用镜像数量")
    parser.add_argument("--sites", default=SITES_FILE, help="站点配置 JSON，默认 sites.json")
    parser.add_argument("--cache-max-age-hours", type=float, default=24.0, help=argparse.SUPPRESS)
...
def run(args: argparse.Namespace) -> int:
    ...
    mirror_url_map = build_mirror_url_map(sites, max(0, args.mirror_limit))
    ...
    print(f"[INFO] 全部站点并发抓取: {len(all_sites)} 个，workers={workers}")
    ...
    with exclusive_path_lock(success_path):
