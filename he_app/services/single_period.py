...
def _dedicated_document_keyword(site: Site, period: int, values: list[str], text: str) -> str | None:
    ...
    if site.site_id in ARTICLE_CONTENT_SITE_IDS and len(values) == 1:
        return (
            "Article/ar_content专属杀合"
            if is_article_content_row(site, text, period, values)
            else None
        )
    ...
    if has_kill_sum_keyword(text, allow_weak=True, value_count=site.value_count):
        return "绝杀一合"
    if "杀合" in text:
        return "杀合"
    return None
...
def _target_values(
    documents: list[str], period: int, allow_weak: bool, value_count: int
) -> tuple[set[str], list[str]]:
    values: set[str] = set()
    lines: list[str] = []
    for line, _has_locator in build_candidate_parts(
        documents, period, allow_weak, value_count
    ):
        extracted = extract_values(line, allow_weak, value_count)
        if len(extracted) != value_count:
            continue
...
def _has_complete_records(documents: list[str], allow_weak: bool, value_count: int) -> bool:
    return bool(build_latest_candidate_parts(documents, allow_weak, value_count=value_count))
...
def _special_segment_is_bounded(site_id: str, text: str, start: int, end: int) -> bool:
    ...
    if site_id == GUCHENG_SITE_ID:
        marker = re.search(r"杀\s*(?:2|二)\s*合\s*数", text)
        if marker is None:
            return False
        section = text[marker.start() :]
        stop = re.search(r"澳门六合彩合数属性|合数属性|copyright", section, re.I)
        limit = marker.start() + stop.start() if stop is not None else len(text)
        name = re.search(r"故城笙声", section)
        row = GUCHENG_ROW_RE.search(text, max(start, marker.start()), min(end, limit))
        return (
            name is not None
            and marker.start() <= marker.start() + name.start() < limit
            and row is not None
            and row.start() >= start
            and row.end() <= limit
        )
...
def build_document_evidence(
    ...
    for index, document in enumerate(documents):
        ...
        if site.site_id == BLACKPEPPER_SITE_ID and len(values) == 1:
            row = normalize_digit_text(normalize_text(detail))
            start = normalized_document.find(row)
            if start < 0 or not is_blackpepper_row(row, period, values[0]):
                continue
            end = start + len(row)
        ...
        for segment, start, end in _candidate_segments(document, period):
            ...
            if site.site_id not in synthetic_detail_sites:
                if _dedicated_document_keyword(site, period, values, segment) is None:
                    continue
                segment_values = extract_values(
                    segment, allow_weak=True, value_count=site.value_count
                )
...
def parse_site_period(site: Site, period: int, documents: list[str]) -> ParseResult:
    ...
    if len(values) != site.value_count or any(not is_valid_sum_value(value) for value in values):
