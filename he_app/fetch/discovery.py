...
def collect_page_documents(session: requests.Session, url: str, timeout: int) -> tuple[list[str], str]:
    ...
    page_decoded = decode_strdecode_blocks(page_html)
    if page_decoded:
        documents.extend(
            SourceDocument(
                text,
        ...
        if script_decoded:
            script_documents.extend(
                SourceDocument(
                    ...
                    document_type="decoded",
                    parent_url=url,
                    authority_id=f"script:{script_url}:decoded:{index}",
